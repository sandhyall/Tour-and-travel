...
export default function PackageBuilder({ data = [], setData }) {
  ...
  const updatePackage = (packageIndex, key, value) => {
    const copy = [...data];
    copy[packageIndex] = { ...copy[packageIndex], [key]: value };
    setData(copy);
  };

  // ग्रुप टियर (Scaling) थप्ने
  const addGroupTier = (packageIndex) => {
    const copy = [...data];
    const tiers = copy[packageIndex].groupPricing || [];
    copy[packageIndex] = {
      ...copy[packageIndex],
      groupPricing: [...tiers, { minPax: "", maxPax: "", pricePerPax: "" }]
    };
    setData(copy);
  };
  ...
  // ग्रुप टियर अपडेट गर्ने
  const updateGroupTier = (packageIndex, tierIndex, key, value) => {
    const copy = [...data];
    const updatedTiers = [...(copy[packageIndex].groupPricing || [])];
    updatedTiers[tierIndex] = { ...updatedTiers[tierIndex], [key]: value };
    copy[packageIndex] = { ...copy[packageIndex], groupPricing: updatedTiers };
    setData(copy);
  };
  ...
  return (
    <div className="flex flex-col gap-6 w-full max-w-4xl mx-auto">
      <div className="flex items-center justify-between border-b border-slate-200 pb-4">
        <h2 className="text-xl font-bold text-slate-800 flex items-center gap-2">
          <Package className="text-indigo-600" size={24} /> Pricing Packages
        </h2>
        <span className="text-xs font-semibold uppercase text-slate-400 bg-slate-100 px-3 py-1 rounded-full">
      ...
      <div className="grid gap-6">
        {data.map((p, i) => (
          <div key={i} className="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm">
            <div className="flex justify-between items-center mb-6">
              <h4 className="font-bold text-slate-700 text-lg">Package Option {i + 1}</h4>
              <button type="button" onClick={() => removePackage(i)} className="text-red-500 hover:bg-red-50 p-2 rounded-lg">
                <Trash2 size={18} />
              </button>
            </div>
            ...
            <div className="space-y-4">
              <input
                type="text"
                placeholder="Package Name (e.g., Luxury, Group Discount)"
                value={p.name || ""}
              ...
              <div className="grid grid-cols-2 gap-4">
                <input type="number" placeholder="Price" value={p.price || ""} className="p-2 border rounded-xl" onChange={(e) => updatePackage(i, "price", e.target.value)} />
                <input type="number" placeholder="Old Price" value={p.oldPrice || ""} className="p-2 border rounded-xl" onChange={(e) => updatePackage(i, "oldPrice", e.target.value)} />
              </div>
              ...
              <div className="border-t pt-4">
                <div className="flex justify-between items-center mb-3">
                  <h5 className="text-sm font-bold">Group Pricing Tiers</h5>
                  <button type="button" onClick={() => addGroupTier(i)} className="text-xs text-indigo-600 font-bold bg-indigo-50 px-3 py-1 rounded-lg">
                    + Add Tier
                  </button>
                </div>
                
                {p.groupPricing?.map((tier, tIdx) => (
                  <div key={tIdx} className="flex items-center gap-2 mb-2 bg-slate-50 p-2 rounded-lg">
                    <input type="number" placeholder="Min" className="w-16 p-1 border rounded" value={tier.minPax} onChange={(e) => updateGroupTier(i, tIdx, "minPax", e.target.value)} />
                    <ArrowRight size={14} />
                    <input type="number" placeholder="Max" className="w-16 p-1 border rounded" value={tier.maxPax} onChange={(e) => updateGroupTier(i, tIdx, "maxPax", e.target.value)} />
                    <input type="number" placeholder="Price/Pax" className="flex-1 p-1 border rounded" value={tier.pricePerPax} onChange={(e) => updateGroupTier(i, tIdx, "pricePerPax", e.target.value)} />
                    <button type="button" onClick={() => removeGroupTier(i, tIdx)} className="text-red-400"><Trash2 size={14} /></button>
                  </div>
                ))}
              </div>
            </div>
      ...
      <button type="button" onClick={addPackage} className="w-full py-4 border-2 border-dashed rounded-2xl text-slate-500 font-bold hover:border-indigo-500">
        <Plus className="inline mr-2" size={20} /> Add New Pricing Package
      </button>
    </div>
  );
}