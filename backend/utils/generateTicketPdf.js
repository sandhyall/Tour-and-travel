...
export const generateTicketPdf = async (booking) => {
  return new Promise((resolve, reject) => {
    try {
      const dir = path.resolve("uploads/tickets");

      if (!fs.existsSync(dir)) {
        fs.mkdirSync(dir, { recursive: true });
      }

      const filePath = path.join(dir, `ticket-${booking._id}.pdf`);
      ...
      doc.fontSize(24).text("Travel Booking Confirmation", {
        align: "center",
      });

      doc.moveDown();

      doc.fontSize(16).text(`Trip: ${booking.trip?.title || "Trip"}`);
      doc.text(
        `Customer: ${booking.buyer.firstName} ${booking.buyer.lastName}`,
      );
      doc.text(`Email: ${booking.buyer.email}`);
      doc.text(`Travel Date: ${new Date(booking.travelDate).toDateString()}`);
