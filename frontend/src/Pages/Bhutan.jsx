    ...
    const fetchBhutanTrips = async () => {
      try {
        ...
        const filtered = data.filter(
          (trip) => trip.country?.toLowerCase() === "bhutan",
        );

        setBhutanTrips(filtered);
      } catch (err) {
        console.error("Error fetching Bhutan trips:", err);
    ...
    return (
      <div className="min-h-[60vh] flex flex-col items-center justify-center bg-white gap-3">
        ...
        <p className="text-xs font-bold tracking-widest uppercase text-stone-400">
          Bhutan Kingdom Packages Loading...
        </p>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-stone-50/50 py-16 px-6">
      <div className="max-w-7xl mx-auto">
        <div className="text-center mb-16">
          <span className="text-xs font-black tracking-[0.25em] text-emerald-600 uppercase block mb-3">
          ...
          <p className="text-stone-600 max-w-2xl mx-auto text-sm md:text-base leading-relaxed">
            Explore Bhutan’s ancient monasteries, cultural heritage, and
            eco-friendly trekking adventures. Experience the unique beauty,
            traditions, and peaceful landscapes of Bhutan.
          </p>
        </div>

        {bhutanTrips.length === 0 ? (
          <div className="text-center py-12 bg-white rounded-3xl border border-stone-200/60 p-8 max-w-md mx-auto">
            <Trees className="w-12 h-12 text-stone-300 mx-auto mb-4" />

            <p className="text-stone-500 font-medium text-sm">
              No trips are currently available for Bhutan.
            </p>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {bhutanTrips.map((trip) => {
              ...
              return (
                ...
                >
                  <div className="relative h-56 w-full overflow-hidden bg-stone-100 shrink-0">
                    ...
                    {/* Category Type Badge */}
                    {trip.categoryType && (
                      <span className="absolute top-4 right-4 bg-emerald-600 text-white font-bold text-[10px] tracking-wider uppercase px-2.5 py-1 rounded-md shadow-sm">
                        {trip.categoryType}
                      </span>
                    )}
                  ...
                  <div className="p-6 flex flex-col flex-grow justify-between">
                    <div>
                      ...
                      {/* Trip Title */}
                      <h3 className="text-lg font-bold text-stone-900 mb-3 leading-snug group-hover:text-emerald-600 transition-colors line-clamp-2 min-h-[3rem]">
                        {trip.title}
                      </h3>
                    </div>
                    ...
                    <div className="mt-4 pt-4 border-t border-stone-100 flex items-center justify-between">
                      ...
                      <div className="w-8 h-8 rounded-full bg-stone-50 text-stone-700 group-hover:bg-emerald-600 group-hover:text-white flex items-center justify-center transition-all">
                        <ArrowRight size={15} />
                      </div>
                    </div>
                  </div>
